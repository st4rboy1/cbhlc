...
export default function DocumentShow({ document, fileUrl }: Props) {
    ...
    const handleVerify = () => {
        if (confirm('Are you sure you want to verify this document?')) {
            router.post(`/registrar/documents/${document.id}/verify`);
        }
    };
    ...
    const handleReject = () => {
        ...
        if (reason && reason.length >= 10) {
            router.post(`/registrar/documents/${document.id}/reject`, { notes: reason });
        } else if (reason) {
    ...
    const handleDelete = () => {
        if (confirm('Are you sure you want to delete this document? This action cannot be undone.')) {
            router.delete(`/registrar/documents/${document.id}`);
        }
    };
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Documents', href: '/registrar/documents' },
